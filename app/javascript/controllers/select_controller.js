...
export default class SelectController extends Controller {
  static targets = ["submitButton", "selectAllButton", "selectNoneButton", "selectionCount"]
  static values = {
    itemType: String,
  ...
  clickOutside(event) {
    ...
    if (event.target.classList.contains('fixed') && event.target.classList.contains('inset-0')) {
      const closeButton = this.element.querySelector('a[data-turbo-frame="modal"]')
  ...
  updateButtonState() {
    ...
    const label = this.itemTypeLabel || this.itemTypeValue || "items"
    ...
    this.selectAllButtonTarget.disabled = allSelected && this.checkboxes.length > 0
    this.selectAllButtonTarget.classList.toggle('opacity-50', allSelected && this.checkboxes.length > 0)
    this.selectAllButtonTarget.classList.toggle('cursor-not-allowed', allSelected && this.checkboxes.length > 0)
  }
