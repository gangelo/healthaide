...
RSpec.describe "Managing foods", type: :system do
  ...
  scenario "User can view details of a food" do
    visit user_foods_path
    
    # Click on a food name to view details
    click_link food_with_qualifiers.food_name
    
    # Verify we're on the right page
    expect(page).to have_current_path(user_food_path(UserFood.find_by(user: user, food: food_with_qualifiers)))
    expect(page).to have_content(food_with_qualifiers.food_name)
    expect(page).to have_content("Organic")
    expect(page).to have_content("Fresh")
  end
  ...
  scenario "User can delete a food", js: true do
    ...
    within("tr", text: food2.food_name) do
      accept_confirm do
        click_button "Delete"
      end
    end
